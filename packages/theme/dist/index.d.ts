...
declare const DefaultLayout: {
    ({ children, defaultLayout, defaultCollapsed, navCollapsedSize, modeToggle, logo, }: LayoutProps): react_jsx_runtime.JSX.Element;
    displayName: string;
};
...
type TableProps<TData extends BaseRecord = BaseRecord, TError extends HttpError = HttpError> = Omit<UseTableProps<TData, TError, TData>, "columns"> & {
    children?: ReactElement<ColumnProps<TData, TError>>[];
    showHeader?: boolean;
};
declare function Table$1<TQueryFnData extends BaseRecord = BaseRecord, TData extends BaseRecord = TQueryFnData, TError extends HttpError = HttpError>({ children, showHeader, ...props }: TableProps<TData, TError>): react_jsx_runtime.JSX.Element;
